...
import co.tryterra.terrartandroid.googlefit.GoogleFitRealtimeCategories
...
class MainActivity : AppCompatActivity(){
    ...
    public override fun onCreate(savedInstanceState: Bundle?) {
        ...
        terraRT = TerraRT(
            ...
            terraRT.initConnection(getToken() ){ }
        }
        ...
        wearOsSwitch.setOnCheckedChangeListener { _, b ->
            if (b){
                GenerateUserToken(XAPIKEY, DEVID, terraRT.getUserId()!!).getAuthToken {
                    terraRT.startRealtime(Connections.WEAR_OS, it!!, setOf(DataTypes.HEART_RATE, DataTypes.STEPS))
                }
            }
            else{
...
fun getToken(): String {
    val serverURL = "https://api.tryterra.co/v2/auth/generateAuthToken"
    ...
    val token = outputJson["token"].toString()
